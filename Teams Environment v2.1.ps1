...
Function Get-TeamsEnvironment
{
            ...
            foreach ($aa in $AAs)
                {
                try {clv ResouceAct } catch{}
                try {clv OperatorID } catch{}
                try {clv Operator } catch{}
                
                foreach ($RA in $aa.ApplicationInstances)
                    {
                        $ResouceAct = Get-CsOnlineApplicationInstance -Identity $ra
                        try {$operatorID = ((Get-CsAutoAttendant -NameFilter $aa.Name | select operator).operator).id}
                        catch{}
                        if ($operatorID)
                            {
                                try {$Operator = (Get-CsOnlineUser -Identity $operatorID | select UserPrincipalName).UserPrincipalName}
                                Catch {}
                            }
                        $detail = New-Object PSObject
            ...
            foreach ($CQ in $CQs)
                {
                    clv agents, OFATarget,  TOATarget, OFATargetUser, TOATargettUser
                    $detail = New-Object PSObject
                    ...
                    foreach ($a in $cq.Agents.objectid)
                        {
                            try {$agent=(get-csonlineuser -Identity $a| select UserPrincipalName).UserPrincipalName + ","}
                            Catch {}
                            $agents +=$agent
                        }
                    ...
                    if ($OFATarget)
                        {
                            $OFATargetUser = (get-csonlineuser -Identity $OFATarget| select UserPrincipalName).UserPrincipalName
                        }
                    ...
                    if ($TOATarget)
                        {
                            $TOATargettUser = (get-csonlineuser -Identity $TOATarget| select UserPrincipalName).UserPrincipalName
                        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUyEWdysgJgyBcfaou3Mi5Mwpa
# WwKgghIHMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# FgQUCUrowNA2XmUxGIpp6AelQr8Ss2owDQYJKoZIhvcNAQEBBQAEggIAjUUvPaY3
# yYPnteGXX21aYzGKPGH2B/JROicfmyf6otggiyzvQQEMufhGXpxqp+SqzZ9cRQqy
# skxIossjzFqx1INidLA0WibNvPptQHhwuZjfzoBa6exFYkR43YuC1cac+LlozNMS
# nOMuh0mrScYgDw8kANTkfZIPQmpVQ4TebdbZ30Fdj5F6tEmcgCcu/Ukw+mShgfHT
# +Djdb/z11EM5Q6ap6dNIAygcGJdUL75TCvkQyicA81ITUN86c/XpJ6yMkt4x940S
# 4byH7whZYPgFnz7obniF8xCx3ziQP7Pzb8xlQPOhuojCSqYhvwJ6PBymcHa+tKeS
# bzkqTWV99ikTZTDXeh4eTl3JgzHh2lAjZ8TDHoZV2oSOn50zpNFvfz/yEjscRvHL
# nqQe8FBDVH12dc8ThhHYD5vvsKLWVyH187FqLFK48QVqAuA8LGIPoxGlSXdcKvAz
# 7EqHRQFb3Tzi0ZwM7SNvbIKIMqqMtAtwBef7zjySJ5Gm4wtewANhmW/M7Mjy1YXU
# 1ISnere22TZ42cfFc0Rj4AAVet1XgUGzvv7zsEgnNQCLn9Tdj04DgnTHwquTgrOV
# +kJXa0UpcDnnY582yrQvRoEImFYn7VFt/NiTwOfHqf9Y7xuvsZTgyVxfqSEMB0sH
# 73LK86swS6eIIwDwiEyEdcBJjIt2nWFQQUE=
